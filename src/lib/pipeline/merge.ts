...
function canMerge(a: Phrase, b: Phrase, config: MergeConfig): boolean {
  ...
  if (a.charCount + 1 + b.charCount > config.maxMergedChars) return false;

  // Would exceed duration limit
  if (b.end - a.start > config.maxMergedDuration) return false;

  return true;
}

/** Merge short/orphan phrases with neighbors until stable. */
export function mergeShortPhrases(phrases: Phrase[], config: MergeConfig): Phrase[] {
  ...
  while (changed) {
    ...
    for (let i = 0; i < phrases.length; i++) {
      ...
      if (isShort && i + 1 < phrases.length) {
        if (canMerge(phrase, phrases[i + 1], config)) {
      ...
      if (isShort && result.length > 0) {
        ...
        if (canMerge(prev, phrase, config)) {
