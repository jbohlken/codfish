#!/usr/bin/env node
// Generates the synthetic media fixture suite for the mediabunny spike into
// test-media/mediabunny-spike/ (gitignored — this script is the source of
...
//
// Usage: node scripts/make-spike-media.mjs   (or: npm run spike:media)
...
const OUT = join(dirname(fileURLToPath(import.meta.url)), "..", "test-media", "mediabunny-spike");
...
writeFileSync(join(OUT, "README.md"), `# mediabunny-spike fixtures

Synthetic files generated by \`npm run spike:media\` (scripts/make-spike-media.mjs).
Regenerate at will — nothing here is checked in. Open the app in dev, hit
Ctrl+Shift+B, and run each file through the spike battery.
